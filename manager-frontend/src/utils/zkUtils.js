...
export async function generateRatingProof(web3, reputationContract, workerAddress, score, orderId) {
  ...
  // Load stored secrets for this order (manager uses 'restaurantOrderSecrets')
  var orderSecrets = JSON.parse(localStorage.getItem('restaurantOrderSecrets') || '{}');
  var secrets = orderSecrets[orderId];
  if (!secrets) throw new Error('Rating secrets not found — was the order accepted from this browser?');
