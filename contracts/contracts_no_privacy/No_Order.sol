...
contract Order {
    ...
    constructor (uint256 _id, address _client, address _restaurant, uint256 _deliveryPrice, uint256 _total, string memory _items) payable {
        id = _id;
        client = _client;
        ...
        clientCode = No_Util.random(block.timestamp+1000, block.difficulty);
        deliverymenCode[0] = No_Util.random(block.timestamp, block.difficulty);
        deliverymenCode[1] = No_Util.random(block.timestamp-33949, block.difficulty);
    ...
    function cancelOrder (address _caller) onlyByMembers(_caller)  external returns (bool) {
        OrderStep _step = No_Util.whoCancel(_caller, step, client, restaurant, deliverymen, block.timestamp, timestamp[step]);
        if (step!=_step) {
    ...
    function getItems(address _caller) external view returns (uint256, string memory, uint256, string memory, address, uint256, uint256, uint256, address){
        ...
        if (_caller==client){
            return (id, items, deliveryPrice, No_Util.getOrderStepValue(step), restaurant, clientCode, timestamp[OrderStep.ORDERED], timestamp[step], address_deliveryman);    
        } else if (_caller == restaurant || _caller==client) {
            return (id,items, deliveryPrice, No_Util.getOrderStepValue(step), address(0), 0, timestamp[OrderStep.ORDERED], timestamp[step], address_deliveryman);   
        } else if (deliverymen.length == 1 && _caller == deliverymen[0]) {
            return (id,items, deliveryPrice, No_Util.getOrderStepValue(step), restaurant, deliverymenCode[0], timestamp[OrderStep.ORDERED], timestamp[step], address_deliveryman);   
        } else if (deliverymen.length == 2 && _caller == deliverymen[1]) {
            return (id,items, deliveryPrice, No_Util.getOrderStepValue(step), restaurant, deliverymenCode[1], timestamp[OrderStep.ORDERED], timestamp[step], address_deliveryman);   
        } else if (step == OrderStep.PREPARATION) {
            return (id, items, deliveryPrice, No_Util.getOrderStepValue(step), restaurant, total, timestamp[OrderStep.ORDERED], timestamp[step], address_deliveryman);    
        } else {
    ...
    function relatedTo(address _caller) external view returns (bool) {
        return No_Util.onlyByMembers(_caller, client, restaurant, deliverymen);
    }
    ...
    function withdraw(address _caller) external payable{
        ...
        (_client, _restaurant, _deliveryman_1, _deliveryman_2) = No_Util.payment(step, address(this), total, deliveryPrice, deliverymen.length);
